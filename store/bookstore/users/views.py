...
def register_handle(request):
    ...
    try:
        ...
        print("error: ", e) # 把异常打印出来
        return render(request, 'users/register.html', {'errmsg': '用户名已存在！'})

    serializer = Serializer(settings.SECRET_KEY, 3600)
    token = serializer.dumps({'confirm':passport.id}) # 返回bytes
    token = token.decode()
    send_mail('宝可梦书城用户激活', '', settings.EMAIL_FROM, [email], html_message='<a href="http://127.0.0.1:8000/user/active/%s/">http://127.0.0.1:8000/user/active/%s/</a>' % (token,token))
...
def upload_pubkey(request):
    pubkey = request.POST.get('publickey')[:-1]
    print(pubkey)

    # add to database
    passport_id = request.session.get('passport_id')
    ps = Passport.objects.add_pubkey(passport_id, pubkey)
